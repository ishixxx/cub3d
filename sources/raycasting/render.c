...
/*   Updated: 2025/07/01 18:08:40 by vgalmich         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	while (x < cub->win_width)
	{
		...
		while (y < cub->win_height)
		{
			...
			if (y < cub->win_height / 2) // si on est a la moitie haute de l'ecran
				draw_pixel(&cub->img, p, create_trgb(0, cub->ceiling)); // coder ces 2 fonctions
			else
				draw_pixel(&cub->img, p, create_trgb(0, cub->floor));
			y++;
		}
		++x;
	}	
...
void	render_3D_scene(t_cub3d *cub)
{
	...
	raycasting(cub);
	mlx_put_img_to_window(cub->mlx_ptr, cub->win_ptr, cub->img[4].mlx_img, 0, 0);
}